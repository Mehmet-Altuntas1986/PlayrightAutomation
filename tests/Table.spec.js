...
// 3) Select multiple products  by using re-usable function ---> it is below in this class
...
// 4) Print all product details using loop
  /* for(let i = 0; i < await rows.count(); i++) 
   {
    const row = rows.nth(i);                // nth ile hangi row oldugunu seciyoruz  nth(0) ilk row
...
// 5) Select all products using loop in all pages      there are 4 tables pages
    ...
    for(let p=0;p<await pages.count();p++) //4
    {
       ...
       if(p>0){                                        //first page icin table numarasina tiklamaya gerek yok
...
///  -------- burda method olusturdu , 3 uncude kullandi
